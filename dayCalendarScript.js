...
var createEvent = (height, top, left, units) => {
  ...
  node.innerHTML = 
  "<span class='title'> Sample Item </span> \ <span class='location'> Sample Location </span>";
...
// append one event to calendar
var createEventPm = (height, top, left, units) => {

  let node = document.createElement("DIV");
  node.className = "eventPm";
  node.innerHTML = 
  "<span class='title'> Sample Item </span> \ <span class='location'> Sample Location </span>";
  ...
  document.getElementById("eventsPm").appendChild(node);
}
...
function getCollisionsPm (eventsPm) {
  ...
  for (var i = 0; i < 24; i ++) {
    var time = [];
    for (var j = 0; j < eventsPm.length; j++) {
      time.push(0);
    }
    collisionsPm.push(time);
  }

  eventsPm.forEach((eventPm, id) => {
    let end = eventPm.end;
    let start = eventPm.start;
    let order = 1;

    while (start < end) {
      timeIndex = Math.floor(start/30);

      while (order < eventsPm.length) {
        if (collisionsPm[timeIndex].indexOf(order) === -1) {
          break;
        }
        order ++;
      }

      collisionsPm[timeIndex][id] = order;
      start = start + 30;
    }

    collisionsPm[Math.floor((end-1)/30)][id] = order;
  });
};
...
horizontal position - pixel offset from left
*/
function getAttributesPm (eventsPm) {

  //resets storage
  width = [];
  leftOffSet = [];

  for (var i = 0; i < eventsPm.length; i++) {
    width.push(0);
    leftOffSet.push(0);
  }

  collisionsPm.forEach((period) => {
...
var layOutDayPm = (events) => {

// clear any existing nodes
var myNode = document.getElementById("eventsPm");
myNode.innerHTML = '';

  getCollisionsPm(eventsPm);
  getAttributesPm(eventsPm);

  eventsPm.forEach((event, id) => {
    let height = (event.end - event.start) / minutesinDay * containerHeight;
    let top = event.start / minutesinDay * containerHeight; 
    let end = event.end;
    let start = event.start;
    ...
    let left = (containerWidth / width[id]) * (leftOffSet[id] - 1) + 10;
    if (!left || left < 0) {left = 10};
    createEventPm(height, top, left, units);
  });
}